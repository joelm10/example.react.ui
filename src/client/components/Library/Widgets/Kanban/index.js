...
const KanbanBoard = ({
    ...
    useEffect(() => {
        const cleanup = createSocketEvents(socket, { onCardMove }, boardEvents);
        return cleanup;
    }, [socket, onCardMove]);
    ...
    const dataUpdateActions = {
        ...
        updateCardState: (updatedColumns) => {
            ...
            logger('info', 'updateCardState()->socket.emit()-> Updating card state with new columns:', updatedColumns);
